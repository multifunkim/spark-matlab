function [Dictionary,output] = spark_vKSVD(...
...
CoefMatrix =zeros(param.K, size(Data,2));
for iterNum = 1:param.numIteration
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%% This part was modifeid %%%%%%%%%%%%%%%%%%%%%
    ...
    for index=1:size(Data,2)
        MDL=[];
        for sp_level=1:param.kmax
            param.L=sp_level;
            % find the coefficients
            if (param.errorFlag==0)
                switch param.SparsecodingMethod
                    case 'OMP'
                        %CoefMatrix = mexOMPIterative2(Data, [FixedDictionaryElement,Dictionary],param.L);
                        CoefVec = OMP([FixedDictionaryElement,Dictionary],Data(:,index), param.L);
                    case 'Thresholding'
                        CoefVec = MPF([FixedDictionaryElement,Dictionary],Data(:,index), param.L);
                end
            else
                %CoefMatrix = mexOMPerrIterative(Data, [FixedDictionaryElement,Dictionary],param.errorGoal);
                CoefVec = OMPerr([FixedDictionaryElement,Dictionary],Data, param.errorGoal);
                param.L = 1;
            end
            % MDL estimation
            if iterNum == 1
                param.totalk = sp_level + param.initk * (size(Data,2)-1);
            else
                param.totalk = sp_level + sum(OPT_L) - OPT_L(index);
            end
            [mdl]=spark_vMDL(Data(:,index), [FixedDictionaryElement,Dictionary], CoefVec, param);
            MDL = [MDL mdl];
        end
        opt_L = find(min(MDL) == MDL);
%         fprintf(['\nThe optimal sparsity level for ' num2str(index) 'th ROI is ' num2str(opt_L) '.\n\n']);
        OPT_L(index) = opt_L;
        param.L = opt_L;
    ...
    rPerm = size(Dictionary,2);%randperm(size(Dictionary,2));
    for j = 1:rPerm
